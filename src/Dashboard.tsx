...
export default function Dashboard() {
  ...
  const [activeNotification, setActiveNotification] = useState<[string, string, string] | null>(null);
  ...
  const doAction = async (action: string, email: string, reason = "Motivul nu a fost specificat") => {
    const token = await getToken();
    ...
    if (response.ok) {
      if(action == 'accept') {
        notify('User acceptat in club', `${email} a fost acceptat in club. Un email i-a fost trimis automat.`, 'SUCCESS')
      } else if(action == 'deny') {
        notify('User nu a fost acceptat in club', `Lui ${email} i-a fost respinsa cererea de a intra in club. Motiv: ${reason}`, 'SUCCESS')
      } else if(action == 'remove') {
        notify('User a fost scos din membri clublui', `${email} a fost scos din membri clubului. Un mail i-a fost transmis automat.`, 'SUCCESS')
      }
      await refreshData();
    } else {
        notify('Eroare', `Am intampinat o eroare incercand actiunea "${action}" pe utilizatorul ${email}.`, 'FAIL')
  ...
  return (
    <motion.div className="min-h-screen w-full flex flex-col items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <AnimatePresence>
        {showReasonPopup && selectedApplicant && (
          ...
          >
            <motion.div
              ...
              exit={{  opacity: 0 }}
            >
              <h2 className="text-xl font-bold mb-4">Motivul respingerii lui {selectedApplicant.name}</h2>
              <textarea
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  onClick={async () => {
                    await doAction("deny", selectedApplicant.email, denyReason || "Motivul nu a fost specificat");
                    setShowReasonPopup(false);
                    setSelectedApplicant(null);
                    setDenyReason("");
                  }}
      ...
      <AnimatePresence>
        {activeNotification && (activeNotification[0] == 'blue' ? (
          <motion.div
          ...
          >
            <div className="w-full flex flex-col p-4 text-black bg-[#00d8ff]">
              ...
              <span className="font-bold text-sm">{activeNotification[2]}</span>
            </div>
        ...
        >
          <div className="w-full flex flex-col p-4 text-white bg-[#ff4d4d]">
            ...
            <span className="font-medium text-sm">{activeNotification[2]}</span>
          </div>
      ...
      <motion.div className="bg-zinc-800/70 w-[90%] max-w-6xl h-[85vh] rounded-2xl p-10 overflow-y-auto shadow-inner backdrop-blur-md border border-cyan-500/20" initial={{ scale: 0.98 }} animate={{ scale: 1 }}>
        <motion.div className="mb-10" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
          <h1 className="text-white font-extrabold text-4xl mb-1">Hackarest Dashboard</h1>
          <h2 className="text-gray-400 text-lg font-medium">Vizualizează cele mai noi aplicații și membri clubului.</h2>
        </motion.div>
          {loading && (
            <motion.div  initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full  flex flex-col items-center justify-center">
              <Ping
                    size="45"
                    speed="2"
                    color="white" 
          ...
          {error && (
             <div className="w-full  flex flex-col items-center justify-center">
                  <h1 className="text-md font-black text-red-500">A aparut o eroare. </h1>
            </div>
          )}

          {applicants?.applicants.length != 0 && applicants !== null && <Section title="Aplicații" list={applicants?.applicants} onAccept={doAction} onReject={setSelectedApplicant} setShowPopup={setShowReasonPopup} calculateAge={calculateAge} />}
          {applicants?.members.length != 0 && applicants !== null &&<Section title="Membri" list={applicants?.members} readOnly calculateAge={calculateAge} onRemove={doAction} />}
        ...
        <div className="mt-10 flex text-center items-center flex-col group">
          ...
          <div className="flex flex-row justify-center gap-4 opacity-0 group-hover:opacity-100 transition">
            <a href="https://github.com/Hackarest/website" target="_blank">
              <span className="text-gray-600 underline hover:text-gray-400 transition cursor-pointer">Repository Web</span>
            </a>
            <a href="https://github.com/Hackarest/api" target="_blank">
              <span className="text-gray-600 underline hover:text-gray-400 transition cursor-pointer">Repository API</span>
            </a>
            <a href="https://github.com/Hackarest/website/blob/main/package.json" target="_blank">
              <span className="text-gray-600 underline hover:text-gray-400 transition cursor-pointer">Versiunea {pkg.version}</span>
            </a>
            <a href="https://github.com/Hackarest/website/blob/main/package.json" target="_blank">
              <span className="text-gray-600 underline hover:text-gray-400 transition cursor-pointer">{Object.keys(pkg.dependencies || {}).length} de librarii folosite</span>
            </a>
          </div>
        </div>
    </motion.div>
...
function Section({ title, list, onAccept, onReject, setShowPopup, readOnly = false, calculateAge, onRemove }: any) {
  return (
    <motion.div className="mt-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
      <div className="w-full flex items-center gap-5 mb-6">
        <div className="h-[1px] bg-cyan-400 flex-1"></div>
        <h2 className="text-white font-semibold text-xl whitespace-nowrap">{title}</h2>
        <div className="h-[1px] bg-cyan-400 flex-1"></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {list?.length ? (
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-300 mb-1"><span className="font-semibold">Email:</span> {person.email}</p>
                  <p className="text-sm text-gray-300 mb-1">
                    <span className="font-semibold">Data nașterii:</span> {person.dob} ({age.years} ani și {age.months === 1 ? "o lună" : `${age.months} luni`})
                  </p>
                  <p className="text-sm text-gray-300 mt-4"><span className="font-semibold">Despre:</span><br />{person.about}</p>
                </div>
                {!readOnly && (
          ...
          <div className="text-white">{readOnly ? "Nu există membri." : "Se încarcă sau nu există aplicații."}</div>
        )}
      </div>
    </motion.div>
  );
